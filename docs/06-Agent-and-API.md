...
The verified Day 22 manual routes are versioned under the `api/v1` prefix: `GET/POST /sales`, `PUT /sales/:id`, `GET/POST /products`, `GET /products/:id`, `PATCH /products/:id`, and `POST /days/:date/coverage`. The application factory receives an authentication adapter and PostgreSQL pool, resolves the owner's one business server-side, and exposes no HTTP input for `business_id` or `actor_user_id`. JSON schemas reject extra properties; money, quantities and versions cross the API as decimal strings; page size is capped at 100 and date ranges at 366 days. HTTP semantics are 400 for malformed JSON, 401/403 for authentication/authorization, 404 for tenant-owned missing IDs, 422 for validation, 409 for version/key conflicts, 429 for limits, and 503 for unavailable dependencies. Mutating requests use a stable idempotency header plus expected versions.

The remaining voice, proposal, analytics, dashboard and provider-session routes below remain proposed. Suggested future application routes use the `api/v1` prefix: voice sessions, proposals, operations, analytics queries and dashboards.
