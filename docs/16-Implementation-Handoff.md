...
Planning notes, the existing Obsidian vault, Graphify artifacts and documentation maintenance scripts exist alongside the implemented Day 20 data foundation, Day 21 mutation service, Day 22 manual ledger API, Day 23 voice session bootstrap and tool gateway, Day 25 analytics endpoint and source transaction drilldown, and a browser dashboard preview with ECharts renderers. The repository now contains the initial PostgreSQL migration, product version migration, voice sessions migration, synthetic catalog seed, exact IDR/USD money module, transactional sale creation, correction and compensating undo, tenant-scoped catalog and sales routes, audited day coverage, ephemeral AssemblyAI session token generation, Fastify voice tool gateway (`get_context`, `propose_sales`, `commit_sales`, `propose_correction`, `commit_correction`, `query_sales`), plus domain, contract and PostgreSQL checks. The browser preview attempts authenticated live analytics through the same-origin API and uses clearly labeled local sample data if that connection is unavailable. Browser voice interaction, production session integration, complete builder behavior and Render deployment remain planned. `npm run graph:serve` opens documentation visualization, not EasyLedger.
...
- `apps/web`: implemented React/Vite dashboard preview with ECharts line and bar charts, Total revenue and Units sold KPI cards, click/keyboard-selected widget properties, conditional live analytics and datum source dialogs; ledger/history, builder persistence controls and microphone/session adapter remain planned.
- `apps/api`: implemented Fastify manual ledger routes, voice tool gateway, dashboard service, `POST /api/v1/analytics/query` and `POST /api/v1/analytics/source-transactions` with an injected authentication adapter; deployment still needs an authenticated browser session integration.
...
- `tests/domain`, `tests/api`, `tests/web` and `tests/database`: implemented deterministic money, mutation, voice, analytics, source authorization and PostgreSQL integration checks. Full browser journey and accessibility acceptance remain planned in [[docs/09-Verification]].
...
The typed web mapper and the ECharts line preserve these states. The table and tooltip distinguish an open no-sale gap, a confirmed zero and unknown-price sales in text. The default unauthenticated preview uses labeled sample data; TASK-25-05 added a conditional live analytics binding. The existing chart options and stylesheet palette were retained.
...
## Implemented TASK-25-05 source transaction drilldown

`POST /api/v1/analytics/source-transactions` (also available at `/api/analytics/source-transactions`) accepts a selected date or product datum, the chart's normalized date and product filters, and its ledger revision. Authentication resolves the tenant on the server. The source service checks the current revision and queries that tenant's non-voided sales in one repeatable-read snapshot; a changed revision returns `409 STALE_QUERY` with refresh guidance. Date and product predicates match the chart query, and bounded keyset pages return sale IDs, dates, product names, quantities, unit prices, exact line revenues, currency and versions. Unknown prices remain null. Malformed cursors, excluded product keys, out-of-range dates and client-supplied tenant identity are rejected.

The browser fetches the four dashboard analytics responses from the same-origin API and displays them as live data only when they share a ledger revision. Without an authenticated API session, it retains the labeled sample fixture. Selecting an ECharts line point or product bar opens the source dialog; visible text tables provide keyboard-accessible datum selection, including gap and zero dates. The dialog requests rows only for live data, validates the response against the selected datum, filters and revision, and supports paging, close/focus restoration and a stale-chart refresh action. Sample data opens a fixture notice without invented source transactions. The existing chart colors, typography and card layout remain in use; the text-table controls and dialog use the same palette.

Verification on 2026-09-25: `npm test` passed 17 domain and 17 API cases, `node --test tests/web/*.test.mjs` passed six cases, and `node --test tests/database/day25_source_transactions_integration.mjs` passed against a disposable PostgreSQL 17-alpine container. The SQL test covered tenant and product/date filtering, voided and unknown-price rows, pagination, and stale revisions. The web production build passed. Desktop browser checks opened the dialog from an ECharts line point and product bar, confirmed that sample values do not claim authorized rows, and used a disposable authenticated fixture to render an exact source transaction. Browser authentication is not configured in the default API server, so the ordinary preview stays in sample mode. The production bundle remains over 500 kB; load time was not measured.
